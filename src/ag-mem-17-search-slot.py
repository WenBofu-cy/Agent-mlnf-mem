...
核心职责: 作为漏斗二中专门承载“信息检索”类任务经验的场景分槽。接收 ag-mem-03 路由的
          信息检索场景经验条目，管理该场景下的五层记忆存储（L1-L5）。在本槽内，复用频次
          （C值）权重自动上调10%，以强化高频检索策略的快速沉淀。同时管理该槽专属的晋升
          阈值与轻度遗忘保护策略参数。不参与认知决策，仅执行信息检索类经验的存储、检索与
          生命周期管理。
...
@dataclass
class WeightConfig:
    c_weight_coefficient: float = 1.1
    l1_to_l2_threshold: float = 0.38
    l2_to_l3_threshold: float = 0.58
    l3_to_l4_threshold: float = 0.78
    l4_to_l5_threshold: float = 0.88
    l1_forget_threshold: float = 0.08
    l2_forget_threshold: float = 0.18
    l3_forget_threshold: float = 0.28
    forget_protection_label: str = "轻度保护"


class SearchSlot:
    def __init__(self):
        self.module_id = "ag-mem-17"
        self.module_name = "信息检索槽"
        self.version = "V1.0"
        self.slot_id = "ag-mem-17"
        ...
        print(f"[{self.module_id}] {self.module_name} {self.version} 初始化完成, C值权重上调10%, 遗忘保护={self._weight_config.forget_protection_label}")
    ...
    def _handle_write(self, request: WriteRequest):
        ...
        entry = request.entry
        # 应用专属权重：C值上调10%
        entry.c_value = min(entry.c_value * self._weight_config.c_weight_coefficient, 1.0)

        # 存储到 L1
        entry_id = f"L1-SEARCH-{uuid.uuid4().hex[:8]}"
        ...
        self._log_event("EXPERIENCE_WRITTEN", {"entry_id": entry_id, "slot": "search"})
        self.state = SlotState.IDLE
...
def demo_main():
    print("=" * 70)
    print("  Agent-mlnf-mem 信息检索槽 (ag-mem-17) 演示")
    print("=" * 70)

    slot = SearchSlot()

    print_separator("STEP 1: 写入信息检索经验（C值上调10%）")
    slot.set_write_request_query(lambda: WriteRequest(
        request_id="REQ-001",
        scene_label="信息检索",
        entry=ExperienceEntry(
            user_id="U001",
            experience_data={"query": "天气查询", "source": "weather_api"},
            c_value=0.5,
            i_value=0.4
        )
    ))
    ...
    if written:
        ...
        print(f"  C值: {written.c_value} (原始0.5 × 1.1)")

    print_separator("STEP 2: 查询信息检索经验")
    ...
    slot.run_slot_cycle()
    print(f"  查询完成, 总条目数: {slot._total_entries}")

    print_separator("STEP 3: 拒绝非信息检索场景的写入")
    slot.set_write_request_query(lambda: WriteRequest(
        request_id="REQ-003",
        scene_label="工具调用",
        entry=ExperienceEntry()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        # TC-M17-01: 正常写入信息检索经验
        print("\n[TC-M17-01] 正常写入信息检索经验")
        try:
            s = setup_slot()
            s.set_write_request_query(lambda: WriteRequest(
                request_id="T01", scene_label="信息检索",
                entry=ExperienceEntry(c_value=0.5, i_value=0.4)
            ))
            s.run_slot_cycle()
        ...
        try:
            s = setup_slot()
            s.set_write_request_query(lambda: WriteRequest(
                request_id="T02", scene_label="信息检索",
                entry=ExperienceEntry(c_value=0.5, i_value=0.4)
            ))
            s.run_slot_cycle()
            written = list(s._entries.values())[0]
            assert written.c_value == 0.55
            print("   ✅ PASS")
            passed += 1
        except Exception as e:
            print(f"   ❌ FAIL: {e}")
        ...
        try:
            s = setup_slot()
            s.set_write_request_query(lambda: WriteRequest(
                request_id="T03", scene_label="对话交互",
                entry=ExperienceEntry()
            ))
            s.run_slot_cycle()
        ...
        try:
            s = setup_slot()
            s.set_write_request_query(lambda: WriteRequest(
                request_id="T04-1", scene_label="信息检索",
                entry=ExperienceEntry(experience_data={"text": "天气查询"}, i_value=0.6)
            ))
            s.run_slot_cycle()
            s.set_query_request_query(lambda: QueryRequest(
                request_id="T04-2", query_conditions={"keywords": ["天气"]}
            ))
            s.run_slot_cycle()
            assert s._total_entries == 1
            print("   ✅ PASS")
